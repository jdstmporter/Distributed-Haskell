...
initialProcess "MASTER" = 
  do { peers <- getPeers
     ...
     ; mapM_ (\ (offset,nid)  -> say(show offset++" : "++show nid)) numberedworkers
  ...
  where 
    ...
    receiveLoop (numIn,numOut) n = 
      let 
        resultMatch = match (\ (nn,x,y) -> return (nn::String,x::Int,y::Int))
      in do { (slavepid,newin,newout) <- receiveWait [resultMatch]
            ; let { x = numIn + newin
            ;       y = numOut + newout }
            ; say("Received "++show newin++" "++show newout++" from "++slavepid)
            ; receiveLoop (x,y) (n-1) }
